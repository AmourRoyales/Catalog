...
export default function NewProductPage() {
  ...
  // Upload one file directly to R2 with progress tracking
  function uploadFile(item: MediaItem, index: number): Promise<MediaItem> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        xhr.onload = () => {
          if (xhr.status === 200) {
            const done: MediaItem = { ...item, progress: 100, status: "done", key, publicUrl };
            setMedia((prev) => prev.map((m, i) => (i === index ? done : m)));
            resolve(done);
          } else {
  ...
  async function handleSave() {
    ...
    try {
      // 1. Upload all media files to R2
      const uploaded: MediaItem[] = [];
      for (let i = 0; i < media.length; i++) {
        if (media[i].status === "done") {
          uploaded.push(media[i]);
        } else {
          const result = await uploadFile(media[i], i);
          uploaded.push(result);
        }
      }

      // 2. Save the product to the database
      // Media records, cover photo first
      const ordered =
        uploaded.length > 0
          ? [uploaded[coverIndex], ...uploaded.filter((_, i) => i !== coverIndex)]
          : [];
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl text-[#16283A]">Add Product</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Fill in details, upload media, save.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-5 items-start">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. 18K white gold halo ring featuring a 1.2ct oval diamond…"
            />
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div
            ...
            className="border-2 border-dashed border-[#3E86C6]/50 rounded-xl p-6 text-center cursor-pointer hover:bg-[#3E86C6]/5 transition"
          >
              ...
              {media.map((m, i) => (
                <div
                  ...
                  className={`relative aspect-square rounded-lg overflow-hidden border-2 cursor-pointer ${
                    coverIndex === i ? "border-[#3E86C6]" : "border-transparent"
                  }`}
                >
          ...
          <p className="text-[11px] text-gray-400 mt-3">
            Click a thumbnail to set it as the cover image.
          </p>
        </div>
      </div>
    </div>
